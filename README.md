...
### UF2 Battlefield App (fresh-start debug target)

Use this target to validate sensors, battery logic, and button behavior without Zigbee/OTA:

```bash
west build -b nrf52840dongle/nrf52840 app_uf2 --build-dir app_uf2/build
```

- Logs are configured for USB CDC ACM console.
- Zigbee and MCUboot OTA are intentionally disabled in this target.
- Default app load offset is set to `0x26000` for common nRF52 UF2 bootloader layouts.
...
- `app_uf2/build/app_uf2/zephyr/zephyr.uf2` — UF2 battlefield firmware (USB drag-and-drop bootloader flow)
